...
struct RenderFinishedSubscription;

impl<H, I> Recipe<H, I> for RenderFinishedSubscription
where
    H: std::hash::Hasher,
{
    type Output = Message;
    ...
    fn stream(
        ...
        use futures::stream;
        Box::pin(stream::once(async { Message::RenderFinished }))
    }
}
...
pub struct State {
    ...
    pub input: String,
    pub output: String,
    pub render_finished: bool, // 追加
}

impl State {
    pub fn subscription(&self) -> Subscription<Message> {
        let clock = time::every(Duration::from_millis(100)).map(Message::Tick);
        let render_finished = if self.render_finished {
            Subscription::from_recipe(RenderFinishedSubscription)
        } else {
            Subscription::none()
        };

        Subscription::batch([clock, render_finished])
    }
